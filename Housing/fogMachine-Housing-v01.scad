...
//	["B", "Rectangle",	[2,-2,0,+box_b/2,10,"outside"],		[2,4,5]		]//aligned to top centre
	...
	["T", "Cylinder",	[10,25,0,0,0,"inside"],				[5,2.5]			], // cutout for led
	["T", "Cylinder",	[-10,25,0,0,0,"inside"],				[5,2.5]			], // cutout for led
	["T", "Square",		[0,0,0,0,0,"inside"],				[5,12]			], // cutout for button
	["N", "Rectangle",	[0,0,-box_l/2,-device_xyz[2]/2,0,"inside"],	[wall_t, 9.5, 9.5] ], // cutout for power cord
	["S", "Rectangle",	[17,-10,0,0,0,"inside"],				[wall_t, 10, 6]	],//cutout for uUSB port
	["S", "Cylinder",	[0,-7,0,0,0,"inside"],				[5,2]	],//cutout for uUSB port
	...
	//["E", "Rectangle",		[15, +1,0,0,0,"inside"],				[wall_t, 10, 6]			],//cutout for uUSB port
...
//	["N", "Cylinder",	[-1,8,0,0,0,"inside"],					[6,3,10]		],//for comparison
	...
	//["T", "Cylinder",	[11,14.4,0,0,30,"inside"],					[5,3,6]		],//fail
	...
	//["T", "Cylinder",	[21.1+4/2,1.14+4/2,-27/2,-40/2,30,"inside"],					[4.33+clearance_xyz[2]-post_tolerance,3,6]		],//post above mag
	//["T", "Rectangle",	[-3,28,0,-40/2,90,"inside"],					[3,2,6]		],//battery retainers across leads
	//["T", "Rectangle",	[-5,13,+27/2,-40/2,00,"inside"],					[3,2,10]		],//batter retainer along side
	//["B", "Rectangle",	[-3,0,0,-32,00,"inside"],					[5,2,36]		],//board retainer along side
	//["B", "Rectangle",	[-17,0,0,-15,00,"inside"],					[5,26,2]		],//board retainer along side
...
text = [//recessed text on faces [face_name, text_to_write, shape_position[x_pos,y_pos,x_offs,y_offs,rotate,align], shape_size[depth,font_height,font_spacing,mirror]] Note: for silly reasons mirror must be 0 or 1 corresponding to false and true in this version
	...
	//["T", "WIMUv3a",			[0,0,0,0,90,"outside"], 					[1,5,1,0]],
	//["E", "Off On",			[0,-5,0,0,0,"outside"], 					[1,3,1,0]],
	//["E", "uUSB",			[15,-5,0,0,0,"outside"], 					[1,3,1,0]],
	//["W", "uSD",			[14,4,0,0,0,"outside"], 					[1,3,1,0]],
	];
...
use<write.scad>;
	...
	make_box(box,corner_radius, corner_sides, lip_h, lip_fit, top_bottom_ratio, mouse_ears, layout, separation, holes, posts, text, items, has_device, box_type);




module make_box(box, corner_radius=3, corner_sides=5, lip_h=2, lip_fit=0, top_bottom_ratio=0.5,mouse_ears=[false],layout="beside", separation=2, holes=[], posts=[], text=[], items=[],has_device=false, box_type="rounded4sides"){
	echo("layout", layout);
		...
		if (layout=="beside"){
			...
			union(){
				translate(v=[(separation+box[0])/2,0,0]){
					half_box(box, corner_radius, corner_sides, lip_h, lip_fit, top_bottom_ratio, which_half="bottom", holes=holes, posts=posts, mouse_ears=mouse_ears, text=text, items=items, box_type=box_type);
...
//					rotate(a=[0,0,-90])translate(v=[-27/2,-40/2,1.88])import("2013_04_04-WIMUv3a_PCB-V0-2_logo.stl");
			...
			echo("stacked");
			half_box(box, corner_radius, corner_sides, lip_h, lip_fit, top_bottom_ratio, which_half="bottom", holes=holes, posts=posts, mouse_ears=mouse_ears, text=text, items=items,has_device=has_device, box_type=box_type);
...
module half_box(box, corner_radius=3, corner_sides=5, lip_h=2, lip_fit=0, top_bottom_ratio=0.5, which_half="bottom", holes=[], posts=[], mouse_ears=[false], text=[], , items=[],has_device=false, box_type="rounded4sides"){
	a_bit=0.01;
	echo("holes",holes);
